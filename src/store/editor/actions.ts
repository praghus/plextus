...
export const changeScale = (scale: number) =>
  ({ type: EDITOR_CHANGE_SCALE, payload: { scale } } as const);

export const changeTool = (tool: string) =>
  ({ type: EDITOR_CHANGE_TOOL, payload: { tool } } as const);
...
export const historyAction = <T>(payload: StringTMap<T>) =>
  ({ type: EDITOR_HISTORY_ACTION, payload } as const);

export const adjustWorkspaceSize = (): AnyAction =>
  changeWorkspaceSize(
    window.innerWidth - RIGHT_BAR_WIDTH,
    window.innerHeight - STATUS_BAR_HEIGHT - FOOTER_HEIGHT,
  );
