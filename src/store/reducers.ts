...
const rehydrateStoreReducer =
  (next: Reducer) => (state: RootState, action: AnyAction) =>
    action.type === APP_REHYDRATE_STORE_SUCCESS
      ? next(action.payload.state, action)
      : next(state, action);
...
export default function createReducer(injectedReducers = {}): Reducer {
  const rootReducer = rehydrateStoreReducer(
    combineReducers({
      ...
      [APP_UNDO_HISTORY_RESOURCE_NAME]: undoHistoryReducerCreator(50),
      ...injectedReducers,
    }),
  );
