...
const MapLayer = ({
    ...
    if (ctx) {
      setPixelTool(
        getImageData(
          ctx,
          TOOL_SIZE,
          selected.tool === TOOLS.ERASER ? [] : selected.color,
        ),
      );
    }
  }, [ctx, selected]);
    ...
    if (visible && isSelected) {
      ...
      switch (selected.tool) {
        ...
        case TOOLS.STAMP:
          if (e.evt.button === 2) {
            ...
            updateLayer(
              x,
              y,
              selected.tool === TOOLS.STAMP ? selected.tileId : null,
            );
          }
          break;
        case TOOLS.PENCIL:
        case TOOLS.ERASER:
          if (gid) {
            if (e.evt.button === 2) {
              const colorData = Object.values(
                ctx.getImageData(localPos.x, localPos.y, 1, 1).data,
              );
  ...
  const onMouseMove = () => {
    ...
    if (isDrawing) {
      const { x: x1, y: y1 } = getPointerRelativePos(
        workspace,
        pointerPosition,
      );
